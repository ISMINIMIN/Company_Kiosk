...
public class MenuInfomation extends Logo {
	...
	public MenuInfomation(MainFrame f) {
		...
		for(int i=0; i<rbtnSize.length; i++) {	
			...
			rbtnSize[i].setBackground(new Color(255,255,255)); // 배경색 지정
			add(rbtnSize[i]);
		...
		for(int i=0; i<rbtnHotIce.length; i++) {
			...
			rbtnHotIce[i].setBackground(new Color(255,255,255));
			add(rbtnHotIce[i]);
	    ...
	    btnAdd.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent evt) {
            	...
            	MenuSelect.lblList.setText(selectedMenu+" ("+selectedSize+" /"+selectedHotIce+" ) x "+count);
            	// 이전 패널로 넘어감
            	F.previousPanel();
            }
        });
	    ...
	    btnAdd.setBackground(new Color(217,242,192));
	    btnAdd.setForeground(new Color(26,116,2));
		...
		btnCancle.setBackground(new Color(217,242,192));
		btnCancle.setForeground(new Color(26,116,2));
